...
class SirenNet(nn.Module):
    def __init__(
        ...
        self.last_layer = Siren(
            dim_in=dim_hidden[num_layers - 1],
            dim_out=dim_out,
            w0=w0,
            use_bias=use_bias,
    ...
    def forward(self, x, mods=None):
        ...
        with profiler.record_function("CAST TUPLE"):
            mods = cast_tuple(mods, self.num_layers)

        for layer, mod in zip(self.layers, mods):
            # print(f"mod.shape : {mod.shape}")

            with profiler.record_function("single siren layer"):
                x = layer(x)

            with profiler.record_function("modulator within the siren"):
                if exists(mod):
...
class Modulator(nn.Module):
    def __init__(self, dim_in, dim_hidden, num_layers):
        ...
        for ind in range(num_layers):
            ...
            self.layers.append(
                nn.Sequential(nn.Linear(dim, dim_hidden[ind]), nn.ReLU())
            )
