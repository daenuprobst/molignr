...
def arg_parse():
    ...
    parser.add_argument('--batch_size', dest='batch_size', type=int,
            help='Batch size.')
    parser.add_argument('--device', dest='cuda', type=int,
...
def set_hyperparameters(prog_args, gnn_type = "gin", dataset = "full_data_knn_4" , latent_dim = 8, epoch = 4000, emb_dim = 2, batch_size = 16, lr = 0.01, gnn_layers = [3, 8, 8, 8]):    
...
def load_model(checkpoint_path):
    ...
    # show top-level module prefixes
    prefixes = sorted({k.split(".")[0] for k in state.keys()})
    print("Top-level modules:", prefixes)

    # show a compact list of keys (first 50)
...
def visualize_latents(prog_args, model, train_loader, epoch = None):
    ...
    pca_file  = pca_dir  / f"{prog_args.gnn_type}_{prog_args.repr}_epoch_{epoch}_dim_{latent_dim}_lr_{prog_args.lr}_pca.png"
    tsne_file = tsne_dir / f"{prog_args.gnn_type}_{prog_args.repr}_epoch_{epoch}_dim_{latent_dim}_lr_{prog_args.lr}_tsne.png"
    umap_file = umap_dir / f"{prog_args.gnn_type}_{prog_args.repr}_epoch_{epoch}_dim_{latent_dim}_lr_{prog_args.lr}_umap.png"
