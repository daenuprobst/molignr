...
from utils import lddt, tm_score, arg_parse, set_hyperparameters, visualize_latents
    ...
    for epoch in range(args.n_epoch):
        ...
        if epoch%10==0:
            if best_model is not None and args.save_output:
                ...
                ppath = os.getcwd() + '/Results'
                saved_path = Path(f"{ppath}/checkpoints/{prog_args.repr}/lr_{prog_args.lr}/"+ f'epoch_{epoch}_dim_{args.latent_dim}_knn_{args.knn}_lr_{prog_args.lr}.pt')
                saved_path.parent.mkdir(parents=True, exist_ok=True)
                torch.save({'epoch': epoch, 
                    'batch': batch_idx, 
