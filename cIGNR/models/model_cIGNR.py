...
from models.layers import GIN_Conv, SAGE_Conv
...
class cIGNR(nn.Module):
    def __init__(self, net_adj, input_card, emb_dim, latent_dim, num_layer, gnn_layers, gnn_type='gin', global_pool='mean', drop_ratio=0.,
                device=device,flag_emb=1, N =2191, representation = "graph"):
        '''
        ...
        if self.representation =="graph":
            print(f"Graph representations in cIGNR")
            self.mlp_coords = nn.Linear(latent_dim, N*3)
        elif self.representation =="node":
            print(f"Node representation in cIGNR")
            self.mlp_coords = nn.Linear(latent_dim, 3)
        else:
            raise Exception("Please specify the representation in latent space - either graph or node")
