...
def arg_parse():
    ...
    parser.add_argument("--repr", type= str, default = None)
...
def load_model(checkpoint_path):
    ...
    print()
    print(f"args.device : {prog_args.device}")
...
def visualize_latents(prog_args, model, train_loader, epoch = None):
    ...
    pca_file  = pca_dir  / f"{prog_args.repr}_epoch_{epoch}_lr_{prog_args.lr}_pca.png"
    tsne_file = tsne_dir / f"{prog_args.repr}_epoch_{epoch}_lr_{prog_args.lr}_tsne.png"
    umap_file = umap_dir / f"{prog_args.repr}_epoch_{epoch}_lr_{prog_args.lr}_umap.png"
