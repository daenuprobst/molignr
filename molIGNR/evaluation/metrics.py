...
def get_indices():
    ...
    u = mda.Universe("/home/binal1/Graphons/cIGNR/evaluation/heavy_chain.pdb")
    idx_N  = []
    idx_CA = []
    idx_C  = []
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda'):
                ...
                loss_coords = torch.nn.functional.mse_loss(coords_pred, batch.x.to(device)) 

                if step%100==0:                  
                    print(f"Reconstruction MSE loss : {loss_coords.item()}")
...
def add_latent_noise(z: torch.Tensor, sigma: float, relative: bool = True):
    """
    z: [B, D] or [D]
    sigma: noise level
    relative: if True, scales sigma by per-sample latent std (more stable)
    """
    if z.dim() == 1:
        z = z.unsqueeze(0)
    ...
    if relative:
        # scale noise by latent scale to make sigma less sensitive to z magnitude
        scale = z.detach().std(dim=-1, keepdim=True).clamp_min(1e-8)
        z_noisy = z + sigma * scale * eps
    else:
        z_noisy = z + sigma * eps
...
def ramachandran_interpolation2(model, dataloader, prog_args, plot_name = "_interpolation_histogram_", N=2191, sigma = 0.3):
    batch_size = prog_args.batch_size
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda'):
                ### interpolate here...
                z, _ = model.encode(batch.x, batch.edge_index, batch.batch)
                z_noisy = add_latent_noise(z, sigma=sigma)


                coords_pred = model.mlp_coords(z_noisy)
                # print(f"coords_pred shape : {coords_pred.shape}")
                coords_pred = coords_pred.view(len(z_noisy), N, 3)

            phi_all_, psi_all_ = get_angles(coords_pred, batch_size=len(z_noisy))
            phi_all_interpolated.extend(phi_all_)
            psi_all_interpolated.extend(psi_all_)
